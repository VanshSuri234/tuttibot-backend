# Simple Installation Guide (INPUT3_LAYER)

Clean, straightforward setup using Audiveris directly without Docker or complex dependencies. This layer provides a minimal Input Layer that:
- Standardizes audio to 44.1 kHz, 16-bit WAV (≤5 channels)
- Converts score PDFs to MusicXML using the Audiveris CLI
- Passes through MIDI/MusicXML without changes
...
# Install minimal requirements for INPUT3_LAYER
pip install -r requirements_input.txt
```

### Step 4: Test the System

```bash
python input_layer.py test_audio.wav test_score.pdf
```
...
├── input_layer.py           # Main input layer code (this repo)
├── requirements_input.txt   # Minimal Python dependencies (this repo)
├── README_INPUT.md          # This guide (this repo)
...
python input_layer.py song.mp3 score.pdf

# No conversion needed
python input_layer.py audio.wav score.xml
python input_layer.py performance.wav composition.mid
```
...
pip install --user -r requirements_input.txt

# Test
python input_layer.py audio.wav score.pdf
```

## 🔗 Relationship to Hybrid Pipeline

The hybrid pipeline in `main_hybrid_v02.py` includes its own input step (`run_input_layer_v02`) that prefers Docker-based Audiveris or an `oemer` fallback for PDF→MusicXML conversion. `INPUT3_LAYER` is an alternative, minimal input layer that uses a direct Audiveris CLI on the host without Docker. You can use `INPUT3_LAYER` when:
- Docker is unavailable (e.g., HPC environments)
- You prefer a minimal dependency footprint
- You already have Audiveris installed system-wide

Outputs from `INPUT3_LAYER` (audio WAV + MusicXML) are compatible with the downstream processing and temporal alignment blocks.
