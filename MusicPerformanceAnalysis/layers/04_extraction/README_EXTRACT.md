...
# Install all dependencies for the extraction layer
...
```
# When run standalone within EXTRACTION_LAYER/
data/
├── processed/
│   ├── input_processed.wav           # Clean, normalized audio
│   ├── input_audio_segments.json     # Temporal segmentation
│   └── input_music_features.json     # Score-derived features
└── original/
  └── xml_score.musicxml            # Original score file
  # OR: score.mid                   # MIDI alternative

# When run as part of the older pipeline (main.py)
../shared_data/
├── processed/
│   ├── input_processed.wav
│   ├── input_audio_segments.json
│   └── input_music_features.json
└── original/
  └── xml_score.musicxml
```
...
# The script will automatically look for files in ../shared_data when run from
# EXTRACTION_LAYER/ (to integrate with other layers). Adjust paths in the script
# if you want to use local data/ instead.
```
...
```
# Default location when running inside EXTRACTION_LAYER/
data/extracted/
...
# If integrated via main.py, outputs may be copied to
../Output/.../03_extraction_layer/
```
...
Below are the core equations behind commonly used features. Symbols follow standard MIR/DSP conventions; $x[n]$ is the audio signal, $X(k,m)$ the STFT at bin $k$, frame $m$ with hop $H$ and window length $N$.

### Fundamental Frequency (YIN/CMNDF)

- Difference function:

$$
D(\tau) = \sum_{n=0}^{N-1-\tau} \big(x[n] - x[n+\tau]\big)^2
$$
...
- Pitch candidate is the first local minimum below a threshold $\theta$; convert lag to frequency:
...
Let $S(k,m) = |X(k,m)|$. The spectral flux novelty is:

$$
  ext{SF}[m] = \sum_k \max\big( S(k,m) - S(k,m-1),\, 0 \big)
$$

Peaks in SF after smoothing and adaptive thresholding correspond to onsets.

### Tempo and Beat Tracking (Autocorrelation/Tempogram)

A common path is to compute a tempogram $G(\omega, m)$ from the onset strength envelope and select the dominant tempo via autocorrelation or Fourier analysis. The selected BPM maximizes periodicity; beat times follow via dynamic programming alignment to the onset curve (librosa implementation).

### Spectral Centroid, Rolloff, Bandwidth

With $S(k,m)=|X(k,m)|$ and frequency bin $f_k$:

$$
  ext{Centroid}[m] = \frac{\sum_k f_k\, S(k,m)}{\sum_k S(k,m)}
$$

Rolloff $f_{\text{ro}}[m]$ is the frequency where the cumulative spectrum reaches a fraction $\alpha$ (e.g., $0.85$):

$$
\sum_{k \le k_{\alpha}} S(k,m) = \alpha \sum_k S(k,m), \quad f_{\text{ro}}[m] = f_{k_{\alpha}}
$$

Bandwidth (second central moment):

$$
  ext{BW}[m] = \sqrt{\frac{\sum_k (f_k - \text{Centroid}[m])^2 S(k,m)}{\sum_k S(k,m)}}
$$

### MFCCs

1) Power spectrum $P(k,m) = |X(k,m)|^2$; 2) Mel filterbank energies $E_i[m] = \sum_k H_i(k) P(k,m)$; 3) Log energies $\log E_i[m]$; 4) DCT-II to obtain coefficients $c_p[m]$:

$$
c_p[m] = \sum_{i=1}^{M} \log E_i[m] \cos\left( \frac{\pi p}{M} (i - 0.5) \right), \quad p=0,\dots,P-1
$$

### Chroma Features

Map frequencies to pitch classes $c \in \{0,\dots,11\}$ via $c = (\operatorname{round}(12\log_2(f/440) + 69)) \bmod 12$ and pool energy across octaves per class to get a 12-D chroma vector per frame.

### RMS Energy and Zero-Crossing Rate

$$
  ext{RMS}[m] = \sqrt{\frac{1}{N} \sum_{n=0}^{N-1} x[n+mH]^2}, \qquad
  ext{ZCR}[m] = \frac{1}{N-1} \sum_{n=1}^{N-1} \mathbb{1}\{\operatorname{sign}(x[n+mH]) \ne \operatorname{sign}(x[n-1+mH])\}
$$

### Harmonic–Percussive Source Separation (HPSS)

Using median filtering on the magnitude spectrogram along time (percussive) and frequency (harmonic) axes to obtain masks $M_H, M_P$; separated components are $X_H = M_H \odot X$ and $X_P = M_P \odot X$ (librosa effects.hpss).
